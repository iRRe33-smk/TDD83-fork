...
function add_apt_card(ad){
    ...
    str += '<h5 class="card-title">'+ad.apartment.address +'</h5>'
...
function add_interests_card(apt){
    ...
    str += '<a class="btn btn-secondary" href="/ad/'+apt.id+'" role="button">Mer info</a>'
...
$(document).ready(function(){
    $(document).on("mypagesroute", function(event) {
        ...
        $.ajax({
            headers: { "Authorization": "Bearer " + JSON.parse(sessionStorage.getItem('auth')).token },
            url: host + "/users"+"/"+current_user.id.toString()+"/apartments",
            type: 'GET',
            contentType: "application/json",
