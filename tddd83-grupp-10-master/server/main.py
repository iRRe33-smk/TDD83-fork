...
class User(db.Model):
    ...
    # lazy: how the data will be loaded in interested_users (so it's not all loaded into the model)
    ...
    def serialize(self):
        return dict(id=self.id, email=self.email, first_name=self.first_name, last_name=self.last_name, bio=self.bio, occupation=self.occupation, customer_id = self.customer_id, is_student = self.is_student)
...
class Apartment(db.Model):
    ...
    ad = db.relationship('Ad', backref='apartment', lazy=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    images = db.relationship('Image', backref='apartment', lazy=True)
    ...
    def serialize(self):
        ...
        return dict(id=self.id, address=self.address, city=self.city, state=self.state, zip_code=self.zip_code, rooms=self.rooms, area=self.area, owner=owner_serialize, images=images_ser)

#Images (the file names for images are stored in the database and can be seen as a multivalued attribute to apartments)
class Image(db.Model):
    __tablename__ = 'image'
    apartment_id = db.Column(db.Integer, db.ForeignKey('apartment.id'), primary_key=True)
...
class Review(db.Model):
    ...
    def __repr__(self):
        return '<Review {}: {} {}>'.format(self.id, self.rating, self.text, self.user_id)

    def serialize(self):
        if self.user is not None:
            user_serialize = self.user.serialize()
        else:
            user_serialize = None
    
        return dict(id=self.id, rating=self.rating, text=self.text, user=user_serialize)
...
def reviews():
    ...
    if request.method == 'GET':
        ...
        request_data = request.get_json()
        new_review = Review(rating = request_data['rating'], text = request_data['text'], user_id = request_data['user'])
...
def specific_user(user_id):
    ...
    if request.method == 'GET':
        ...
        for apartment in new_user.apartments: 
            #Apartment.user_id can not be NULL so we have to delete all apartments owned by a user that is about to be deleted
...
def show_reviews(user_id):
    ...
    for review in new_user.reviews:
        review_list.append(dict(id=review.id, rating=review.rating, text=review.text))
    return jsonify(review_list)
